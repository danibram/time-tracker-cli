...
var sumarize = exports.sumarize = function sumarize(search, tasks, rate, full, format) {
    ...
    tasks.forEach(function (task, index) {
        var description = task.task.description() && task.task.description() !== '' ? task.task.description() : task.name;
        task = task.task;

        var duration = task.getSeconds();
        total += duration;

        table.push([description, (0, _moment2.default)(task.getStartDate()).format(format), (0, _utils.humanParseDiff)(duration)]);
    });
    ...
    if (full) {
        ...
        if (rate) {
            ...
            table.push([{ rowSpan: 2, content: _colors2.default.red('Search:') + ' "' + search + '"', vAlign: 'center' }, { rowSpan: 2, content: _colors2.default.red('Total time'), vAlign: 'center' }, { rowSpan: 2, content: (0, _utils.humanParseDiff)(total), vAlign: 'center' }], []);
        }
    }
