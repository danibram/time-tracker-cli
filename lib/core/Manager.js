...
var Manager = function () {
    ...
    _createClass(Manager, [{
        ...
        key: 'getTasksMd',
        value: function getTasksMd(key) {
            var _this8 = this;

            var timings = this.tasks[key].timings;
            var body = "| Inicio | Fin | Horas | Subtotal |\n";
            body += "| -----  | --- | -----:| -------: |\n";
            var total = 0;
            timings.forEach(function (k) {
                var start = new Date(k.start);
                var stop = new Date(k.stop);
                total += stop - start;
                body += "|" + _this8.dateFormat(start) + "|" + _this8.dateFormat(stop) + "|" + _this8.hourFormat(stop - start) + "|" + _this8.hourFormat(total) + "|\n";
            });
            body += "| | |\n";
