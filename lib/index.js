...
_commander2.default.command('export [task_string]').description('Export the tasks in a JSON').alias('e').option('-f --format [format]').option('-s --start [start date]').option('-e --end [end date]').action(function (key, options) {
    //console.log("Key: "+ key)
    //console.log("Format: /" + options.format+ "/")
    //console.log("Options: "+ JSON.stringify(options.start))
